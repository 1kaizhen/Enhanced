# Folio — Bookmarks, Tasks & Now-Playing
...
- **Tasks panel** — add to-dos with low/medium/high priority, attach Pomodoro-style timers, filter by All / Active / Done. Collapses into a compact strip that always keeps the "+ Task" button visible.
...
- **Local-first** — everything lives in `chrome.storage.local`. Nothing leaves your browser.
...
media-content.js     Content script injected into every page — picks the primary <video>/<audio>,
                     reports state, accepts play/pause/next/previous commands
...
- **Render model** — full re-render on every mutation. State lives in module-level arrays; mutations call `saveAll()` / `saveTodos()` then re-render.
